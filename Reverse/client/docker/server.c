...
void myencrypt(uint32_t* v, uint32_t* key) {
...
void mydecrypt(uint32_t* v, uint32_t* key) {
    uint32_t v0 = v[0], v1 = v[1];
...
void run_validation()
{
    ...
    bool correct;
    const char* key = "Lab_Sign_1N_Key@";
    mydecrypt((uint32_t*)(answer + 0), (uint32_t*)key);
    mydecrypt((uint32_t*)(answer + 8), (uint32_t*)key);
    myencrypt((uint32_t*)(answer + 16), (uint32_t*)key);
    myencrypt((uint32_t*)(answer + 24), (uint32_t*)key);
    if (!memcmp(answer, global_data.Data, 32))
        correct = true;
    else
        correct = false;
    
    operation_dispatcher(OPT_EXEC, 14, NULL);
    global_data.Data[255] = correct;
    if (correct)
    {
        FILE* f = fopen("./flag", "r");
        if (f)
        {
            ...
            fgets(flag_buffer, 0x100, f);
            fclose(f);
            f = NULL;
            strcpy(global_data.Data, flag_buffer);
        }
        else
            strcpy(global_data.Data, "Failed to open flag file, please contact admin for help.");
    }
